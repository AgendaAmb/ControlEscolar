...
//Teams
use App\Mail\SendTeamsMeeatingInformation;
...
class InterviewController extends Controller
{
        ...
        try {
            ...
            if ($interview2->url == null) {
                ...
                elseif (str_contains($int_room->site, 'Teams') ? true : false) {
                    Interview::where('id', $request->id)->update(['confirmed' => true, 'url' => 'En linea por Teams']);
                }
                else {
                    ...
                    if ($User->type == "externs" || $User->type == "students" || $User->type == "Comunidad AA") {
                        ...
                        elseif (str_contains($int_room->site, 'Teams') ? true : false) {
                            ...
                            Mail::mailer($servicio_correo)
                                ->cc($mail_academic_program)
                                ->to($user_mail)
                                ->send(new SendMeeatingInformation($interview2, $user_data, $archive->announcement->academicProgram, $request->room, $archive->id, $url_LogoAA));
            ...
            if (count($archives)) {
                foreach ($archives as $archive1) {
                    ...
                    if ($academic_program != null) {
                        ...
                        try {
                            if ($user_data != null) {
                                // Mail::mailer($servicio_correo)->to('ulises.uudp@gmail.com')->send(new UpdateDocumentsInterview($user_data, $academic_program, $archive->id,$url_LogoAA ));
    ...
    public function documentsForInterviewShow(Request $request, $archive_id)
    {
        ...
        try {
            ...
            if (sizeof($archive->interviewDocuments) < 2) {
                try {
                    $required_documents = RequiredDocument::where('type', 'interview')->get();
                    $ids_rd = [];
                    foreach ($required_documents as $rd) {
