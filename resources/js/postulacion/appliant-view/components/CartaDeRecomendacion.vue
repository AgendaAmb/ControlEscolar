<template>
  ...
  <div class="col-12 my-2">
    ...
    <div class="row">
      ...
      <div v-for="(my_email, index) in emails" :key="index" class="form-group col-6" >
        <!-- Ya existe la carta acorde al indexado -->
        <div v-if="recommendationLetterPerIndex(index) === true">
          <valida-carta-recomendacion :email="recommendation_letters[index].email_evaluator"
          :recommendation_letter="recommendation_letters[index]" :appliant="appliant" :archive_id="archive_id"
          :academic_program="academic_program" :images_btn="images_btn" :index="index + 1" :status_checkBox="status_checkBox">
          </valida-carta-recomendacion>
        </div>
        <!-- No existe carta, tendra que ser rellenada -->
        <div  v-else> 
          <valida-carta-recomendacion :email="my_email.email" :archive_id="archive_id" :appliant="appliant"
          :academic_program="academic_program" :index="index + 1" :images_btn="images_btn" :status_checkBox="status_checkBox">
        </valida-carta-recomendacion>
        </div>
      </div>
    </div>
